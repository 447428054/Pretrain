...
class TrainingDatasetRoBERTa(TrainingDataset):
    ...
    def sentence_span_process(self, text):
        """单个文本的处理函数
        流程：分词，然后转id，按照mask_rate构建全词mask的序列
              来指定哪些token是否要被mask
        """
...
if __name__ == '__main__':
    ...
    workers = 40
    max_queue_size = 4000
    dict_path = '/home/root1/lz/pretrain/tensorflow/chinese/chinese_roberta_wwm_large_ext_L-24_H-1024_A-16/vocab.txt'
    ...
    if model == 'roberta':
        ...
        for i in range(10):  # 数据重复10遍
            TD.process(
                corpus=tqdm(some_texts('/home/root1/lz/roberta-NER-unlabel/raw_text/unlabel_train.txt')),
                record_name='../corpus_tfrecord-unlabel-ngram/corpus.%s.tfrecord' % i,
                workers=workers,
                max_queue_size=max_queue_size,
            )
